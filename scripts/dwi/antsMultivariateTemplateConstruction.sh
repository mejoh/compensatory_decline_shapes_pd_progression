...
# ANTS=/opt/ANTs/20150828/build/bin/ANTS
# WARP=/opt/ANTs/20150828/build/bin/WarpImageMultiTransform
...
fle_error=0
for FLE in $ANTS $WARP $N4 $PEXEC $PBS #$SGE $XGRID $SLURM
  do
    if [[ ! -x $FLE ]];
      then
        echo
...
if [[ "$RIGID" -eq 1 ]];
    ...
    for (( i = 0; i < ${#IMAGESETARRAY[@]}; i+=$NUMBEROFMODALITIES ))
        ...
        if [[ $DOQSUB -eq 1 ]];
            ...
            then
            id=`qsub -N antsrigid -v ANTSPATH=$ANTSPATH $QSUBOPTS -l nodes=1:ppn=2 -l walltime=20:00:00 -l mem=8gb $qscript | awk '{print $1}'`
            jobIDs="$jobIDs $id"
            sleep 0.5
        elif [[ $DOQSUB -eq 2 ]];
            then
...
while [[ $i -lt ${ITERATIONLIMIT} ]];
    ...
    for (( j = 0; j < ${#IMAGESETARRAY[@]}; j+=$NUMBEROFMODALITIES ))
        ...
        if [[ $DOQSUB -eq 1 ]];
            ...
            echo -e "$SCRIPTPREPEND" > $qscript
            echo -e "$exe" >> $qscript
            id=`qsub -N antsdef${i} -v ANTSPATH=$ANTSPATH -l nodes=1:ppn=2 -l walltime=20:00:00 -l mem=8gb $QSUBOPTS $qscript | awk '{print $1}'`
            jobIDs="$jobIDs $id"
            sleep 0.5
        elif [[ $DOQSUB -eq 2 ]];
            then
