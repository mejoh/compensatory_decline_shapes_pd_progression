---
title: "Longitudinal comparisons of task-"
...
clinvars <- c('pseudonym', 'TimepointNr', 'YearsToFollowUp', 'ParticipantType',
              ...
              'Up3OfTotal', 'Up3OfBradySum',
              'Up3OfRigiditySum', 'Up3OnTotal', 'Up3OnBradySum','Up3OnRigiditySum',
              'z_MoCA__total', 'z_CognitiveComposite2',
...
# Set time-invariant covariates to their baseline values
baseline_covs <- dfClin %>%
        filter(TimepointNr==0) %>%
        select(pseudonym,ParticipantType,Age,Gender,NpsEducYears,PrefHand,MostAffSide)
dfClin <- dfClin %>% 
        select(-c(Age,Gender,NpsEducYears,PrefHand,MostAffSide)) %>%
...
## Correlation between on- and off-state clinical scores

```{r CLIN-ON-VS-OFF-PROG, echo=FALSE, warning=TRUE, message=TRUE}
...
## Medication effects on bradykinesia progression rates

```{r CLIN-TIMExMED, echo=FALSE, warning=TRUE, message=TRUE}
...
## Medication effects on bradykinesia progression rates, controlling for baseline cognition and medication dosage

```{r CLIN-TIMExMED-C1, echo=FALSE, warning=TRUE, message=TRUE}
...
```{r CLIN-COG-PROG, echo=FALSE, warning=TRUE, message=TRUE}
...
# Correlation between MoCA increase and bradykinesia progression
tmp.dMOCA <- tmp.MOCA %>%
        filter(Medication=='On') %>%
        pivot_wider(id_cols = c('pseudonym','Age','Gender','YearsSinceDiag','NpsEducYears'),
                    names_from = TimepointNr,
...
## Progression of medication dosage

```{r CLIN-MED-PROG, echo=FALSE, warning=TRUE, message=TRUE}
...
## Bradykinesia progression, co-varying out progression of cognitive impairment and medication dosages

```{r CLIN-BRADY-PROG-C1, echo=FALSE, warning=TRUE, message=TRUE}
