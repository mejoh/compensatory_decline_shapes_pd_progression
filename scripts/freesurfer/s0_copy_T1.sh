...
# Copy T1w scans of each subject and session. Note the 'if' statement, which
# ensures that off-state sessions from the PIT study is not processed. 
# (Un)comment the 'slices' commands for QC output.
...
cd $bidsdir 
# In case copied T1w images are only used for templates
